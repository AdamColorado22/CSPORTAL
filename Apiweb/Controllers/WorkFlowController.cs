    ...
    public class WorkFlowController : Controller
    {
        ...
        public JsonResult ListaMuestras()
        {
            ...
            .Select(dataRow => new Muestras
            {
                ...
                TP_CANTIDAD = dataRow.Field<decimal>("TP_CANTIDAD"),
                PROCESO_ACTUAL = dataRow.Field<string>("PROCESO_ACTUAL"),
                FECHA_INICIO = dataRow.Field<string>("FECHA_INICIO"),
                TIEMPO_PROCESO = dataRow.Field<decimal>("TIEMPO_PROCESO"),
                TIEMPO_TOTAL = dataRow.Field<decimal>("TIEMPO_TOTAL"),
        ...
        public JsonResult Guardar(WORKFLOWB2B objeto)
        {
            try
            {
              ...
              string sql = @" EXEC TER_WOKFLOWB2B_INICIO @ID,@TIPO,@TITULO,@PASO_ACTUAL,@TIPO_SERVICIO,@IDCLIENTE,@NOTAS,@USUARIO,@ARCHIVO";
        ...
        public JsonResult GuardarArt(TER_W_WORFLOW_CARTICULO objeto)
        {
            try
            {
                ...
                string sql = @" EXEC TER_SP_W_CREACION_ART @ID,@TIPO,@TITULO,@PASO_ACTUAL";
                using (TERMOAPPEntities db = new TERMOAPPEntities())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
        ...
        public JsonResult GuardarMue(Muestras objeto)
        {
            try
            {
                ...
                string sql = @" EXEC TER_SP_W_CREACION_MUE @ID,@DUI,@IDCLIENTE,@NOMBRE_PRODUCTO,@TIPO_PROYECTO,@FECHA_REQUERIDA";
                using (TERMOAPPEntities db = new TERMOAPPEntities())
                {
                    using (var dbContextTransaction = db.Database.BeginTransaction())
                    {
        ...
        {    
            try
            {
                ...
                switch (objeto.PASO_ACTUAL)
                {
                    ...
                    //    break;
                    case 50:
                        sql2 = String.Format("UPDATE TER_W_WORKFLOWB2B SET UNIDAD_VENTA='{0}',	MARGEN='{1}',	NECESITA_MUESTRA='{2}' ,LOTE_INDUSTRIAL ='{3}' WHERE ID ={4} ", objeto.UNIDAD_VENTA, String.IsNullOrEmpty(objeto.MARGEN) ? "" : objeto.MARGEN, objeto.NECESITA_MUESTRA, objeto.LOTE_INDUSTRIAL, objeto.ID);
                        break;
        ...
        public JsonResult SiguienteArt(TER_W_WORFLOW_CARTICULO objeto)
        {
            try
            {
                ...
                {
                    sql2 = String.Format("UPDATE TER_W_WORFLOW_CARTICULO SET FORMULA_EXT='{0}' ,CANTIDAD_MAX='{1}' WHERE ID={2}",
                      String.IsNullOrEmpty(objeto.FORMULA_EXT) ? "" : objeto.FORMULA_EXT, String.IsNullOrEmpty(objeto.CANTIDAD_MAX.ToString()) ? "" : objeto.CANTIDAD_MAX.ToString(),objeto.ID);
                }
        ...
        public JsonResult SiguienteMue(Muestras objeto)
        {
            try
            {
                ...
                if (objeto.PASO_ACTUAL.Equals(10))
                {
                    sql2 = String.Format("UPDATE TER_W_WORKFLOW_MUESTRA SET P_EQUIPO_TEC='{0}' ,EXP_MATERIALES='{1}' ,MUESTRA='{2}' ,CODIGO_U_MUESTRA='{3}' ,MAESTR_ING='{4}' ,DUI_MUESTRA='{5}' ,CANTIDAD={6},FORMULA='{7}' ,ESTRUCTURA='{8}'    WHERE ID={9}",
                      ...
                      , String.IsNullOrEmpty(objeto.CANTIDAD.ToString()) ? 0 : objeto.CANTIDAD, String.IsNullOrEmpty(objeto.FORMULA.ToString()) ? "" : objeto.FORMULA.ToString(), String.IsNullOrEmpty(objeto.ESTRUCTURA.ToString()) ? "" : objeto.ESTRUCTURA.ToString(), objeto.ID);
                }
                ...
                if (objeto.PASO_ACTUAL.Equals(30))
                {
                    sql2 = String.Format("UPDATE TER_W_WORKFLOW_MUESTRA SET TP_CANTIDAD={0} , COMENTARIOS='{1}' WHERE ID={2}",
                      String.IsNullOrEmpty(objeto.TP_CANTIDAD.ToString()) ? 0 : objeto.TP_CANTIDAD, String.IsNullOrEmpty(objeto.COMENTARIOS) ? "" : objeto.COMENTARIOS, objeto.ID);
                }
                ...
                if (objeto.PASO_ACTUAL.Equals(50))
                {
                    sql2 = String.Format("UPDATE TER_W_WORKFLOW_MUESTRA SET PASO='{0}' , CANTIDAD_LOTE='{1}' WHERE ID={2}",
                      String.IsNullOrEmpty(objeto.PASO.ToString()) ? "" : objeto.PASO, String.IsNullOrEmpty(objeto.CANTIDAD_LOTE.ToString()) ? "" : objeto.CANTIDAD_LOTE, objeto.ID);
                }
                string sql = @" EXEC TER_W_SIGUIENTE @ID,@TIPO,@PASO_ACTUAL,@USUARIO";
                using (TERMOAPPEntities db = new TERMOAPPEntities())
                {
        ...
        public void envioCorreo(WORKFLOWB2B objeto)
        {
            ...
            switch (objeto.PASO_ACTUAL)
            {
                ...
                case 20:
                    ...
                    mensaje = " El usuario " + usuario + " El usuario ha creado numero de EDUI <br> " + objeto.DUI + " " +
                        "<br> Correspondiente al proyecto: " + objeto.TITULO+
                        "<br> Fecha: " + now.ToString("dd/MM/yyyy");
                    break;
                case 30:
                    para = correos.ObtenerCorreo(objeto.TIPO, objeto.PASO_ACTUAL);//ricardo bonilla, jose pedro
                    if (objeto.CELULA_TECNICA.Equals("SI")) 
                    {
                        asunto = String.Format("Consulta cvt del proyecto: {0}", objeto.TITULO);
                        mensaje = " Es necesario completar la información  "+objeto.INFORMACIONREQ+ " <br> el EDUI: " + objeto.DUI + " <br> del Proyecto <br> " + objeto.TITULO + " <br> Fecha: " + now.ToString("dd/MM/yyyy");
                     break;
                    }
                ...
                case 40:
                    ...
                    mensaje = "Se ha completado la información <br>  para el DUI: " + objeto.DUI + "<br>para el Proyecto " + objeto.TITULO + " <br> Fecha: " + now.ToString("dd/MM/yyyy");
                    break;
                case 50:
                    para = correos.ObtenerCorreo(objeto.TIPO, objeto.PASO_ACTUAL);//
                    asunto = String.Format("Cotizacion Generada {0}-{1}", objeto.IDCLIENTE, objeto.TITULO);
                    mensaje = " El usuario " + usuario + " ha iniciado  un nuevo proyecto <br> Referencia de Cotizacion" + objeto.COTIZACION + "<br>" + objeto.TITULO + " <br> Fecha: " + now.ToString("dd/MM/yyyy");
                    break;
                case 60:
                    ...
                    mensaje = " Referencia de cotizacion " + objeto.COTIZACION + "<br> Notas:  "+objeto.NOTASREQ
                        + "<br> Correspondiente al DUI:  " + objeto.DUI +
                        "<br> Que pertenece al proyecto:  " + objeto.TITULO +
                        " <br> Fecha: " + now.ToString("dd/MM/yyyy");
                   break;
                case 80:
                    para = correos.ObtenerCorreo(objeto.TIPO,50);//
        ...
        public void envioCorreo3(Muestras objeto)
        {
            ...
            switch (objeto.PASO_ACTUAL)
            {
                ...
                case 0:
                    ...
                    mensaje = " El usuario " + usuario + " ha iniciado  un nuevo proyecto <br> " + objeto.PRODUCTO + " <br> Fecha Requerida: " + objeto.FECHA_REQUERIDA;
                    break;
                case 10:
                    para = correos.ObtenerCorreo("MUE", (Int32)objeto.PASO_ACTUAL);//mangandi
                    asunto = String.Format("Liberacion Muestra {0}-{1} ", "", objeto.IDCLIENTE, objeto.PRODUCTO);
                    mensaje = " El usuario " + usuario + " ha iniciado  un nuevo proyecto <br> " + objeto.PRODUCTO +" <br> Fecha: " + now.ToString("dd/MM/yyyy");
                    break;
                case 20:
                    para = correos.ObtenerCorreo("MUE", (Int32)objeto.PASO_ACTUAL);//mangandi
                    asunto = String.Format("Liberacion Muestra {0}-{1} ", "", objeto.IDCLIENTE, objeto.PRODUCTO);
                ...
                case 30:
                    para = correos.ObtenerCorreo("MUE", (Int32)objeto.PASO_ACTUAL);//mangandi
                    asunto = String.Format("Liberacion Muestra {0}-{1} ", "", objeto.IDCLIENTE, objeto.PRODUCTO);
                    mensaje = " El usuario " + usuario + " ha iniciado  un nuevo proyecto <br> " + objeto.PRODUCTO +
                        "<br> OT:" + objeto.OT+
                         "<br> Cantidad:" + objeto.CANTIDAD +
                        " <br> Fecha: " + now.ToString("dd/MM/yyyy");
                    break;
                case 40:
                    para = correos.ObtenerCorreo("MUE", (Int32)objeto.PASO_ACTUAL);//mangandi
                    ...
                    mensaje = " El usuario " + usuario + " ha iniciado  un nuevo proyecto <br> " + objeto.PRODUCTO +
                        "<br> Estado:" + objeto.APROBADA +
                         "<br> Fecha Aprobada:" + objeto.FECHA_VALIDACION_C;
                    break;
